...
For `REVIEW`, `OPTIMIZE`, or `FULL`, read [layered-control.md](references/layered-control.md). These modes must freeze a global manuscript contract before section, subsection, paragraph, or sentence work. If the user has not selected review depth, ask the granularity question from that reference; never choose silently.
